...
public class GoController {
    ...
    void GoButtonOnAction(ActionEvent event) {
        ...
        if(Math.pow(x,2)+Math.pow(y,2)==Math.pow(25,2) && y==-Math.abs(x) || Math.pow(x,2)+Math.pow(y,2)==Math.pow(25,2)&& y==Math.abs(x)||y==-Math.abs(x)&&y==Math.abs(x)){
            result.setText("На границе");
        }else if (Math.pow(x,2)+Math.pow(y,2) < Math.pow(25,2) && y < -Math.abs(x) && y < Math.abs(x)){
            result.setText("Да");
        } else
            result.setText("Нет");
    }
